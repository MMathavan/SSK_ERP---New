...
namespace SSK_ERP.Controllers
{
    ...
    public class SalesEInvoiceController : Controller
    {
        ...
        {
            var master = db.TransactionMasters.FirstOrDefault(t => t.TRANMID == id && t.REGSTRID == 20);
        ...
        public ActionResult Print(int id)
        {
            ...
            if (result != null)
            {
                ...
                if (model != null && !string.IsNullOrWhiteSpace(model.QrCodePath))
                {
                    var qrPath = model.QrCodePath.Trim();
                        ...
                        && !isPhysicalFilePath)
                    {
                        ...
                        {
                            model.QrCodePath = "https://my.gstzen.in" + qrPath;
                        }
                        else
                        {
                            model.QrCodePath = "https://" + qrPath;
                        }
                    }
                }

                return View("~/Views/SalesEInvoice/Print.cshtml", model ?? result.Model);
            }
...
", diagConn))
        ...
        public async Task CInvoice(int id = 0)/*10rs.reminder*/
        {
            ...
            if (master != null && !string.IsNullOrWhiteSpace(master.ACKNO))
            {
                ...
                Response.Write("Acknowledge Number already present. Upload is not allowed.");
                return;
            }
            ...
            string _SconnStr = ConfigurationManager.ConnectionStrings["SSK_DefaultConnection"].ConnectionString;
            ...
            while (reader.Read())
            {
                ...
                discamt = 0;// Convert.ToDecimal(reader["DISCAMT"]);

                roff_amt = Convert.ToDecimal(reader["TRANROAMT"]);

                custgid = 1;// Convert.ToInt32(reader["CUSTGID"]);
                switch (custgid)
                {
                    case 6:
                        suptyp = "SEZWP";
            ...
            using (var httpClient = new HttpClient())
            {
                using (var request = new HttpRequestMessage(new HttpMethod("POST"), "https://my.gstzen.in/~gstzen/a/post-einvoice-data/einvoice-json/"))
                {
                    // Replace with your authorization code de3a3a01-273a-4a81-8b75-13fe37f14dc6 
                    ...
                    //var zresponse = await httpClient.PostAsync(url, content);
                    if (response != null)
                    {
                        ...
                        //var jsonObject = $.parseJSON(zjsonString);
                        //console.debug(jsonObject.responseText.amount);

                        //string timeZone = data["status"].Value<string>();
                        //string zmsg = data["message"].Value<string>();

                        //jsonString = jsonString.Replace("<br>", "~");
                        //jsonString = jsonString.Replace("=>", "!");
                        //var zparam = jsonString.Split('~');
                        ...
                        if (status == 1)
                        {
                            ...
                            //imgUrl = "";// "~fnsjdjszvw/a/invoices/bb3a9424-5aef-48b9-aeeb-38feee471b03/einvoice/qrcode";
                            var imageFileUrl = "";
                            var newimageurl = "";

                            //if (imgUrl != "")
                            //{
                            //    imageFileUrl = imgUrl.Substring(7, imgUrl.Length - 7);
